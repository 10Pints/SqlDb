...
';
   ...
   BEGIN TRY
      ...
      SELECT
          @schema = a
         ,@table_nm = b
      FROM dbo.fnSplitPair2(dbo.fnDeSquareBracket(@q_table_nm), '.');
...
',@sql;
            ...
            EXEC sp_log 1, @fn, '120:@len:',@len;

            IF @len = 0
            BEGIN
               EXEC sp_log 1, @fn, '122: @len = 0 so setting len = 8000';
               SET @LEN = 8000;
            END
         ...
         EXEC sp_log 1, @fn, '110: field ty is ',@fld_ty, ' LEN: ', @len;
         EXEC sp_assert_not_equal 0, @len, ' field length should never be zero', @fn=@fn;
         -- Add the field info to the FieldInfo table
         UPDATE FieldInfo SET
             ty    = @fld_ty
            ,[len] = @len
         WHERE id = @fld_id;
