...
-- Rtn:              test.hlpr_081_sp_merge_delegate_tbl
-- Description: test helper for the dbo.sp_merge_delegate_tbl routine tests 
--
...
-- Tests:
--===========================================================================
CREATE PROCEDURE [test].[hlpr_081_sp_merge_delegate_tbl]
    @tst_num                 VARCHAR(50)
   ,@display_tables          BIT
...
BEGIN
   DECLARE
    @fn                      VARCHAR(35)    = N'hlpr_081_sp_merge_delegate_tbl'
...
';
      ...
      BEGIN
         BEGIN TRY
            EXEC sp_log 1, @fn, '010: Calling the tested routine: dbo.sp_merge_delegate_tbl';
            ------------------------------------------------------------
            EXEC @act_RC = dbo.sp_merge_delegate_tbl
               ;
  
            SELECT @act_row_cnt = COUNT(*) FROM Delegate;
            ------------------------------------------------------------
            EXEC sp_log 1, @fn, '020: returned from dbo.sp_merge_delegate_tbl';
         ...
         IF @exp_row_cnt IS NOT NULL EXEC tSQLt.AssertEquals @exp_row_cnt, @act_row_cnt,'085 row_cnt';
         IF @exp_RC      IS NOT NULL EXEC tSQLt.AssertEquals @exp_RC     , @act_RC     ,'086 RC';
