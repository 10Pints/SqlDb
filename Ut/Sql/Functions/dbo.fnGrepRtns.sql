...
-- Description: lists db objects
-- @types is a comma separated list in a string used in the IN() clause
--   DEFAULT_CONSTRAINT
...
-- Type Code	Object type	Description of the Object Type
-- ------------------------------------------------------------------------------------
...
-- -- ==================================================================================
CREATE FUNCTION [dbo].[fnGrepRtns](@schema NVARCHAR(50), @tyfiltr  NVARCHAR(50))
RETURNS TABLE
AS
  RETURN
   SELECT 
    so.[name]
   ,so.[type]        AS obj_ty
   ,so.[type_desc]   AS obj_ty_desc
   ,so.create_date   AS crt_dt
   ,ss.[name]        AS [schema]
      FROM
         sys.objects    so 
      ...
      WHERE
          ss.[name] = @schema
      AND so.[type] LIKE CONCAT(@tyfiltr, '%');
GO
