...
GO
/*
Type  Object type	                        Description of the Object Type
...
*/
CREATE VIEW [dbo].[SysObjects_vw]
AS
SELECT TOP 2000
    DB_NAME() as db_name
    ,OBJECT_SCHEMA_NAME(object_id)  AS schema_nm
    ,[name]
    ,[type]                         AS ty_code
    ,[type_desc]                    AS ty_nm
    ,dbo.fnFormatDate(create_date)  AS created
    ,dbo.fnFormatDate(modify_date)  AS modified
FROM sys.objects
    WHERE
     [type] IN ('AF', 'C', 'D', 'F', 'FN', 'FS', 'FT', 'IF', 'IS', 'P', 'PC', 'PK'
     , 'TA', 'TF', 'TR', 'TT', 'U', 'UQ', 'V','X')
     AND [name] NOT IN ('sp_alterdiagram','sp_dropdiagram','sp_helpdiagramdefinition','sp_helpdiagrams','sp_renamediagram','sp_upgraddiagrams')
ORDER BY schema_nm, [type], [name]
/*
SELECT * FROM [dbo].SysObjects_vw WHERE [ty_code] IN('C', 'D', 'F', 'FN', 'FS', 'FT', 'IF', 'IS', 'P', 'PC', 'PK'
     , 'TA', 'TF', 'TR', 'TT', 'U', 'UQ', 'V','X')
*/
GO
