...
BEGIN
   ...
   -- Add the TST parameters
   INSERT INTO @t (line) 
   SELECT
      CONCAT
      (
         @tab2, iif(ordinal=1, ' ', ',')
      , dbo.fnPadRight(test.fnGetParamWithSuffix(ordinal), @max_prm_len+4), ' = ', iif(is_chr_ty=1, '''''', '0'))
   FROM test.ParamDetails
   WHERE tst_ty = 'TST';

   -- Add the INP parameters
   INSERT INTO @t (line) 
   SELECT
      CONCAT
      (
   ...
   -- Add the EXP parameters
   INSERT INTO @t (line) 
   SELECT
      CONCAT
      (
         @tab2, ','
        ,dbo.fnPadRight(test.fnGetParamWithSuffix(ordinal), @max_prm_len+4), ' = NULL'
      )
   FROM test.ParamDetails
   WHERE tst_ty = 'EXP';
   RETURN;
...
INSERT INTO @t (line) VALUES
   ...
   ,( CONCAT( @tab2, dbo.fnPadRight(',@tst_key_nm' , @max_prm_len+5), ' = NULL'))
   ,( CONCAT( @tab2, dbo.fnPadRight(',@tst_key_val', @max_prm_len+5), ' = NULL'))
   ,( CONCAT( @tab2, dbo.fnPadRight(',@tst_fld_nm' , @max_prm_len+5), ' = NULL'))
*/
/*
   -- Add an exp variable for each out parameter
   INSERT INTO @t (line) 
   SELECT
      CONCAT
      (
         @tab2, ','
        ,dbo.fnPadRight(CONCAT('@exp_',param_nm), @max_prm_len+4), ' = NULL')
   FROM test.ParamDetails
   WHERE parameter_mode = 'INOUT';
*/
